...
export default async function handle(req, res) {
    if (req.method === 'POST') {
        try {
            ...
            const [rechercheIdClient] = await bdd.query('SELECT idClient as idCli FROM client WHERE mail = ?', [mailClient])
            const idClient = rechercheIdClient[0].idCli

            const [rechercheSoldeClient] = await bdd.query('SELECT solde FROM client WHERE idClient = ?', [idClient])
            ...
            if (soldeClient > sommeCommande) {
                await bdd.execute('INSERT INTO commande (idCommande, idClient, montant) VALUES (?, ?, ?)', [idCommande, , idClient, sommeCommande])
                ...
                await bdd.execute('UPDATE client SET solde = solde - ? WHERE idClient = ?', [sommeCommande, idClient])
