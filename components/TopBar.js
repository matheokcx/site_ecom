...
export default function TopBar({ articleCherche, setArticleCherche, faireRecherche, panierVisible, setPanierVisible, panier, userMail }) {
    ...
    return (
        <>
            <div className='w-full h-1/6 flex flex-row items-center justify-center gap-8 bg-blue-600 text-white'>
                ...
                <Image src='/profilPicture.png' width='40' height='40' alt='pp' className='hover:cursor-pointer' onClick={() => routeur.push(`/profilClient?liste=${panier}&userMail=${userMail}`)} />
            </div>
        </>
    )
}