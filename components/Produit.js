...
export default function Produit({ element, panier, setPanier }) {
    ...
    return (
        <>
            <div className='w-3/8 lg:w-2/12 h-3/5 flex flex-col items-center gap-3 rounded-lg p-3 border-2 border-gray-400 border-solid text-black'>
                <Image src={element.path} width='80' height='80' className='w-3/5 h-2/6 rounded-lg' onClick={() => routeur.push(`/${element.nom}?prix=${element.prix}&mail=${mail}`)} alt='image produit' />
